  ...
  function applyFilters(updatedFilters) {
    ...
    if (updatedFilters.agentIds?.length > 0) {
      newFilteredTickets = newFilteredTickets.filter((ticket) =>
        updatedFilters.agentIds.includes(ticket.agentId)
      );
    }
    if (updatedFilters.customerIds?.length > 0) {
      newFilteredTickets = newFilteredTickets.filter((ticket) =>
        updatedFilters.customerIds.includes(ticket.createdBy)
      );
    }
    if (updatedFilters.priorities?.length > 0) {
      newFilteredTickets = newFilteredTickets.filter((ticket) =>
        updatedFilters.priorities.includes(ticket.priority)
      );
    }
    if (updatedFilters.statuses?.length > 0) {
      newFilteredTickets = newFilteredTickets.filter((ticket) =>
        updatedFilters.statuses.includes(ticket.status)
      );
    }
    ...
    <>
      <Grid container spacing={2}>
        <Grid item xs={12} lg={isFiltersOpen ? 9 : 12}>
          <Paper
            sx={{ padding: "1em", paddingRight: 0, flexGrow: 1 }}
            elevation={2}
          >
            <Typography
              variant="h5"
                  ...
                  >
                    ...
                    <Button
                      disabled={
                        tableOptions.page >=
                        Math.ceil(
                          tableOptions.count / tableOptions.rowsPerPage
                        ) -
                          1
                      }
                      onClick={() => setCurrentPage(tableOptions.page + 1)}
