...
export const LoginRegister = () => {
    ...
    runWithProgress({ data: credentials, func: loginToApi }).then(
      (accessToken) => {
        if (accessToken) {
          ...
          setUser({
            ...decodedUser,
            RoleAssignments: JSON.parse(decodedUser.RoleAssignments),
          });
          navigate("/");
        } else {
          setError(true);
  ...
  function register() {
    ...
    if (!registrationFormData.companyId) {
      setErrors((prev) => ({ ...prev, companyId: true }));
    }
    ...
    if (!registrationFormData.passwordConfirmation) {
      setErrors((prev) => ({ ...prev, passwordConfirmation: true }));
    }
    if (
      registrationFormData.password != registrationFormData.passwordConfirmation
    ) {
      setErrors((prev) => ({
        ...prev,
      ...
      <form>
        ...
        >
          <Paper sx={{ borderTopRightRadius: 0, borderBottomRightRadius: 0 }}>
            <Stack
              sx={{
                minWidth: { xs: "70vw", sm: "50vw", md: "40vw" },
                minHeight: "80vh",
                paddingInline: "2em",
              }}
              spacing={4}
                ...
                <>
                  ...
                  >
                    <TextField
                      variant="outlined"
                      error={error}
                      label="Email (Utilisez spadmin@gmail.com)"
                      sx={{ minWidth: "60%" }}
                      onChange={(event) =>
                        setCredentials((prev) => ({
                          ...prev,
                    ...
                    <TextField
                      variant="outlined"
                      error={error}
                      label="Mot de passe(Utilisez Admin123#)"
                      type="password"
                      sx={{ minWidth: "60%" }}
                      onChange={(event) =>
                        setCredentials((prev) => ({
                ...
                <>
                  ...
                  >
                    ...
                    <FormControl sx={{ minWidth: "60%" }} size="small">
                      ...
                      >
                        {companies?.map((c) => (
                          <MenuItem value={c.id} key={`company-${c.id}`}>
                            {c.name}
                          </MenuItem>
                        ))}
                      </Select>
